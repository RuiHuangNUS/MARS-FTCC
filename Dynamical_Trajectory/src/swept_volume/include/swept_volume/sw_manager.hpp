...
public:
    ...
    std::map<std::string, std::function<BasicShape *(const Config &)>> shapeConstructors = {
        ...
        // {"sdRectangle", [](const Config &conf)
        //  { std::cout << "\033[35m=======sdRectangle======= "<< "\033[0m" << std::endl;
        // return new sdRectangle(conf); }},

        {"sdRectangle0", [](const Config &conf)
         { std::cout << "\033[35m=======sdRectangle0======= "<< "\033[0m" << std::endl;
        return new sdRectangle0(conf); }},
        {"sdRectangle90", [](const Config &conf)
         { std::cout << "\033[35m=======sdRectangle90======= "<< "\033[0m" << std::endl;
        return new sdRectangle90(conf); }},
        // {"sdRectangle45", [](const Config &conf)
        //  { std::cout << "\033[35m=======sdRectangle45======= "<< "\033[0m" << std::endl;
        // return new sdRectangle45(conf); }},

        {"MARS3x2", [](const Config &conf)
         { std::cout << "\033[35m=======MARS3x2======= "<< "\033[0m" << std::endl;
        return new MARS3x2(conf); }},
        
        {"sdRectangleLong0", [](const Config &conf)
         { std::cout << "\033[35m=======sdRectangleLong0======= "<< "\033[0m" << std::endl;
        return new sdRectangleLong0(conf); }},
        {"sdRectangleLong90", [](const Config &conf)
         { std::cout << "\033[35m=======sdRectangleLong90======= "<< "\033[0m" << std::endl;
        return new sdRectangleLong90(conf); }},
        {"sdRectangleLong45", [](const Config &conf)
         { std::cout << "\033[35m=======sdRectangleLong45======= "<< "\033[0m" << std::endl;
        return new sdRectangleLong45(conf); }},

        // {"Lthick", [](const Config &conf)
        //  { std::cout << "\033[35m=======Lthick======= "<< "\033[0m" << std::endl;
        // return new Lthick(conf); }},
...
public:
    ...
    // zhang added
    // Record data to csv
    inline void saveYawAndTransToCSV(const std::string &filename, double delta, double yaw, double myyaw, const Eigen::Vector3d &trans)
    {
        std::ofstream file;
        file.open("../yaw_trans_data.csv", std::ios::out | std::ios::app); // Open in append mode
        ...
        if (file.tellp() == 0)
        {
            file << "Time, Yaw, MyYaw, TransX, TransY, TransZ\n";
        }

        // Write yaw and trans information
        file << std::fixed << std::setprecision(6) << delta << "," << yaw << "," << myyaw << ","
    ...
    inline void process(const Trajectory<TRAJ_ORDER> &traj)
    {
        ...
        if (delta > 0.0 && delta < traj.getTotalDuration())
        {
            ...
            double Myyaw = atan2(Rt(1, 0), Rt(0, 0)) * 57.63; 
            // std::cout << "MyYaw: " << Myyaw << std::endl;

            // saveYawAndTransToCSV("yaw_trans_data.csv", delta, yaw, Myyaw, trans);
