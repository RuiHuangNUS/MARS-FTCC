...
void publish_cmd(Vector3d p, Vector3d v, Vector3d a, Vector3d j, double y, double yd)
{
  ...
  std::cout << "cmd" << cmd.position.x  << std::endl;
  ROS_INFO("CMD3:");
...
void cmdCallback(const ros::TimerEvent &e)
{
  ...
  if ((time_now - heartbeat_time_).toSec() > 0.5)
  {
    ...
    // ROS_INFO("CMD:", pos, vel, acc, jer, yaw_yawdot.first, yaw_yawdot.second);
  }
  ...
  if (t_cur < traj_duration_ && t_cur >= 0.0)
  {
    ...
    std::cout << "qqqqq " << std::endl;
    ROS_INFO("CMD1:%.3f,%.3f,%.3f,%.3f,%.3f,%.3f", pos, vel, acc, jer, yaw_yawdot.first, yaw_yawdot.second);
#if FLIP_YAW_AT_END or TURN_YAW_TO_CENTER_AT_END
    finished = false;
#endif
  }
  ...
  else if (t_cur >= traj_duration_)
  {
    ...
    publish_cmd(pos, vel, acc, jer, yaw_yawdot.first, yaw_yawdot.second);
    std::cout << "qqqqq " << std::endl;
    ROS_INFO("CMD2:%.3f,%.3f,%.3f,%.3f,%.3f,%.3f", pos, vel, acc, jer, yaw_yawdot.first, yaw_yawdot.second);
  }
#endif
  ...
  else if (t_cur >= traj_duration_)
  {
    ...
    publish_cmd(pos, vel, acc, jer, yaw_yawdot.first, yaw_yawdot.second);
    // std::cout << "qqqqq " << std::endl;
    // ROS_INFO("CMD3:%.3f,%.3f,%.3f,%.3f,%.3f,%.3f", pos, vel, acc, jer, yaw_yawdot.first, yaw_yawdot.second);
  }
#endif
