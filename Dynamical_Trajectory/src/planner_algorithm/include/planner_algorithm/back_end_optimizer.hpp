...
public:
 ...
 static inline double costFunctionLmbmParallel(void *ptr,
                                          ...
                                          const int n)
    {
        ...
        // zhang added
        // dynamic feasibility term with optimal attitude
        double dur = obj.step_traj.getDurations().sum();
        double total_yaw = 0.0;     
        double grad = 0.0;
        double yawTarget = M_PI / 2;

        for (double i = 0; i < dur - 1e-4; i += 0.1)
        {
            double nyaw = obj.step_traj.getPos(i)[2];
            // std::cout << "nyaw: " << nyaw * 57.63 << std::endl;

            double delta_y = obj.step_traj.getPos(i)[1] - obj.step_traj.getPos(i)[1];
            double delta_x = obj.step_traj.getPos(i)[0] - obj.step_traj.getPos(i)[0];
            if( delta_x != 0)
            {
            ...
            total_yaw = total_yaw + nyaw + (grad - M_PI/2) + yawTarget;
        }
        total_yaw = 0.5* fabs(total_yaw);
