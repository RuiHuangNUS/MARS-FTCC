...
void PlannerManager::generateTraj(vector<Vector3d> path)
{
  ...
  if (mid_ret && ret)
  {
    ...
    // double current_time = std::min(recent_traj.getTotalDuration(), ros::Time::now().toSec());
    auto current_pos = recent_traj.getPos(ros::Time::now().toSec());
    std::cout <<  current_pos(0) << current_pos(1) << current_pos(2) << std::endl;

    double yaw = current_pos(2);
